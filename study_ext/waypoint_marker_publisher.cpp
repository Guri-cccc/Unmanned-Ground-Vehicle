...
void createGlobalLaneArrayOrientationMarker(const autoware_msgs::LaneArray& lane_waypoints_array)
{
  ...
  lane_waypoint_marker.header.frame_id = "odom";
  lane_waypoint_marker.header.stamp = ros::Time::now();
  lane_waypoint_marker.type = visualization_msgs::Marker::LINE_STRIP; //ARROW
  lane_waypoint_marker.action = visualization_msgs::Marker::ADD;
  ...
  for (auto lane : lane_waypoints_array.lanes)
  {
    ...
    for (int i = 0; i < static_cast<int>(lane.waypoints.size()); i++)
    {
      geometry_msgs::Point pointBuf;
      pointBuf = lane.waypoints[i].pose.pose.position;
      lane_waypoint_marker.points.push_back(pointBuf);  
      std_msgs::ColorRGBA colorBuf;
      colorBuf.a = 1, colorBuf.r = 1, colorBuf.g = 1, colorBuf.b = 1;
      lane_waypoint_marker.colors.push_back(colorBuf);
      lane_waypoint_marker.id = i;
    }
    // lane_waypoint_marker.pose = lane.waypoints[i].pose.pose;
    tmp_marker_array.markers.push_back(lane_waypoint_marker);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber lane_array_sub = nh.subscribe("/waypoint_lane", 10, laneArrayCallback);
